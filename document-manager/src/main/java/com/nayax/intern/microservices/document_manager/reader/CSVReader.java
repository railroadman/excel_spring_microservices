...
import com.nayax.intern.microservices.document_manager.helper.CSVHelper;
...
@Component
public class CSVReader implements DocReader{
    @Override
    public List<DocLine> readFile(InputStream is) throws IOException, DocumentFormatException {
    try {
           List<DocLine> listDocLine = CSVHelper.readFiles(is);
           return listDocLine;
        } catch (Exception ex) {
